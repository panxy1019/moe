#!/usr/bin/env python3
"""Deep semi-intrusive MoE-ROM v3 experiment with history context and router smoothness.

The model extends v2 with sliding-window history features and temporal router smoothness:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("/root/moe_rom_v3"))
    parser.add_argument("--experiment-name", default="deep_moe_v3")
    ...
    parser.add_argument("--patience", type=int, default=85)
    parser.add_argument("--rollout-steps", type=int, default=20)
    parser.add_argument("--train-rollout-steps", type=int, default=5)
...
def evaluate_model(
    ...
    smooth_mask = (prev_ids >= 0) & (arrays["re"][prev_ids] == arrays["re"][sample_ids])
...
def rollout_loss_torch(
    ...
    for _ in range(args.train_rollout_steps):
        ...
        base_x = make_features_torch(a_cur, b, rhs_g, re, ph, args.phase_harmonics)
        x = make_history_features_torch(
            base_x,
            a_cur,
            b,
            rhs_g,
            arrays_t["hist_idx"][current],
            arrays_t,
        )
...
def train_one_split(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, num_train, args.batch_size):
            ...
            smooth_mask = (prev_ids >= 0) & (arrays_t["re"][prev_ids] == arrays_t["re"][batch_ids])
...
def write_summary_md(path: Path, result: Dict[str, object]) -> None:
    ...
    for item in result["results"]:
        ...
        lines.append(
            ...
            f"{deep['router_temporal_smooth_mse']:.6g} | {roll['relative_l2_mean']:.6g} | "
...
def main() -> None:
    ...
    out = {
        "scheme": "deep_physical_context_history_smooth_router_moe_dual_head",
