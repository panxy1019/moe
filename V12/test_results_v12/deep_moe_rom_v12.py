#!/usr/bin/env python3
"""Deep semi-intrusive physics-anchored operator-space MoE-ROM v12.

V12 follows MoE lessons from GShard/Switch/Mixtral/DeepSeekMoE: top-k routing,
explicit load regularization, and isolated shared experts that are always active.
For ROM dynamics, the shared physical operator is the semi-intrusive Galerkin
RHS; learned shared experts capture common closure, and routed experts capture
regime-local operator closures. RK4 remains the velocity time integrator.

    [a_t, b_t, Re, phase, physical descriptors, history]
      -> shared encoder h_t
      -> regime router pi_i(h_t, descriptors)
      -> shared physical operator + learned shared experts + routed local experts
      -> f_i = Galerkin RHS + learned local closure_i
      -> f = shared_common + sum_i pi_i f_i
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("/root/moe/V12/test_results_v12/results")
    )
    parser.add_argument("--experiment-name", default="deep_moe_v12")
...
class OperatorSpaceMoEROM(nn.Module):
    def __init__(
        ...
        op_dim = out_dim + pressure_dim
        self.experts = nn.ModuleList(
            [
                nn.Sequential(
                    nn.LayerNorm(hidden_dim),
                    MLP(hidden_dim, expert_hidden, op_dim, dropout),
                )
                for _ in range(num_experts)
            ]
        )
        if self.num_operator_spaces > 0:
            self.shared_operator_spaces = nn.ModuleList(
                [
                    nn.Sequential(
                        nn.LayerNorm(hidden_dim),
                        MLP(hidden_dim, expert_hidden, op_dim, dropout),
    ...
    def forward(
        ...
        for block in self.refine_blocks:
            h = h + block(h)
        logits = self.router(torch.cat([h, x], dim=1))
        gate = topk_router_from_logits(logits, self.top_k, self.gate_floor, self.temperature)
        expert_stack = torch.stack([expert(h) for expert in self.experts], dim=1)
        routed_op = torch.einsum("be,bed->bd", gate, expert_stack)
        if self.num_operator_spaces > 0:
            shared_stack = torch.stack(
                [expert(h) for expert in self.shared_operator_spaces], dim=1
            )
            shared_gate = torch.softmax(self.shared_mixer(h), dim=-1)
            shared_op = torch.einsum("bs,bsd->bd", shared_gate, shared_stack)
            denom = max(self.routed_scale + self.shared_scale, 1.0e-6)
            op = (self.routed_scale * routed_op + self.shared_scale * shared_op) / denom
        else:
            op = routed_op
        rhs_std = op[:, : self.out_dim]
        pressure_std = op[:, self.out_dim : self.out_dim + self.pressure_dim]
        return rhs_std, pressure_std, [gate], expert_stack
...
def shared_operator_analysis(
    ...
    if len(sample_ids) == 0:
        return {}
    if model.num_operator_spaces <= 0 or model.shared_mixer is None:
    ...
    with torch.no_grad():
        ...
        for block in model.refine_blocks:
            h = h + block(h)
        gate = torch.softmax(model.shared_mixer(h), dim=-1)
...
def write_summary_md(path: Path, result: Dict[str, object]) -> None:
    ...
    lines = [
        ...
        (
            ...
            f"targets plus a pressure `{settings['pressure_target']}` branch using one shared regime router. "
...
def main() -> None:
    ...
    out = {
        "scheme": "weighted_l2_deep_moe_rom_v12_re50_300_operator_space_relative_surrogate_rk4",
