#!/usr/bin/env python3
"""Deep semi-intrusive physics-aware hierarchical MoE-ROM v13.

V13 follows the supplied research plan: enlarged encoder/expert capacity,
Top-2 sparse routed experts, always-on shared experts, separate velocity and
pressure routers, and physics-aware expert heads with linear and low-rank
quadratic state terms. RK4 remains the velocity time integrator.

    [a_t, b_t, Re, phase, physical descriptors, history]
      -> shared encoder h_t
      -> velocity router pi^u_i and pressure router pi^p_i
      -> velocity/pressure routed experts + always-on shared experts
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("/root/moe/V13/test_results_v13/results")
    )
    parser.add_argument("--experiment-name", default="deep_moe_v13")
    ...
    parser.add_argument("--num-experts", type=int, default=24)
...
class OperatorSpaceMoEROM(nn.Module):
    def __init__(
        ...
        self.num_operator_spaces = max(0, num_operator_spaces)
        self.num_shared_experts = self.num_operator_spaces
        self.top_k = top_k
        ...
        self.velocity_router = nn.Sequential(
            nn.LayerNorm(hidden_dim + in_dim),
            nn.Linear(hidden_dim + in_dim, hidden_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
            nn.Linear(hidden_dim, num_experts),
        )
        self.pressure_router = nn.Sequential(
            nn.LayerNorm(hidden_dim + in_dim),
            nn.Linear(hidden_dim + in_dim, hidden_dim),
            nn.SiLU(),
            nn.Dropout(dropout),
        ...
        self.velocity_experts = nn.ModuleList(
            [
                PhysicsAwareExpert(
                    hidden_dim,
                    out_dim,
        ...
        self.pressure_experts = nn.ModuleList(
            [
                PhysicsAwareExpert(
                    hidden_dim,
        ...
        if self.num_operator_spaces > 0:
            self.velocity_shared_experts = nn.ModuleList(
                [
                    ...
                    for _ in range(self.num_operator_spaces)
                ]
            )
            ...
            self.velocity_shared_mixer = nn.Sequential(
                nn.LayerNorm(hidden_dim),
                nn.Linear(hidden_dim, self.num_operator_spaces),
            )
            self.pressure_shared_mixer = nn.Sequential(
                nn.LayerNorm(hidden_dim),
                nn.Linear(hidden_dim, self.num_operator_spaces),
            )
        else:
    ...
    def _shared_mix(
        ...
        shared_stack = torch.stack([expert(h, state) for expert in experts], dim=1)
        shared_gate = torch.softmax(mixer(h), dim=-1)
        return torch.einsum("bs,bsd->bd", shared_gate, shared_stack)
    ...
    def forward(
        ...
        vel_logits = self.velocity_router(router_in)
        pressure_logits = self.pressure_router(router_in)
        vel_gate = topk_router_from_logits(
            vel_logits, self.top_k, self.gate_floor, self.temperature
        )
        pressure_gate = topk_router_from_logits(
            pressure_logits, self.top_k, self.gate_floor, self.temperature
        )
        rhs_op, velocity_stack = self._sparse_mix(
            self.velocity_experts, h, a_state, vel_gate, return_stack=True
        )
        pressure_op, _ = self._sparse_mix(
            self.pressure_experts, h, pressure_state, pressure_gate, return_stack=False
        )
        if self.num_operator_spaces > 0:
            rhs_shared = self._shared_mix(
                self.velocity_shared_experts, self.velocity_shared_mixer, h, a_state
            )
            pressure_shared = self._shared_mix(
                self.pressure_shared_experts,
                self.pressure_shared_mixer,
                h,
                pressure_state,
            )
            denom = max(self.routed_scale + self.shared_scale, 1.0e-6)
            rhs_op = (self.routed_scale * rhs_op + self.shared_scale * rhs_shared) / denom
            pressure_op = (
                self.routed_scale * pressure_op + self.shared_scale * pressure_shared
            ) / denom
...
def evaluate_model(
    ...
    with torch.no_grad():
        rhs_std, pressure_std, gates, _ = model(x)
    rhs_pred = (
...
def shared_operator_analysis(
    ...
    if len(sample_ids) == 0:
        return {}
    if (
        model.num_operator_spaces <= 0
        or model.velocity_shared_mixer is None
        or model.pressure_shared_mixer is None
    ):
    ...
    with torch.no_grad():
        ...
        for block in model.refine_blocks:
            h = h + block(h)
        vel_gate = torch.softmax(model.velocity_shared_mixer(h), dim=-1)
        press_gate = torch.softmax(model.pressure_shared_mixer(h), dim=-1)
    vel_np = vel_gate.detach().cpu().numpy()
    press_np = press_gate.detach().cpu().numpy()
    gate_np = 0.5 * (vel_np + press_np)
    entropy = -np.sum(gate_np * np.log(gate_np + EPS), axis=1)
    vel_entropy = -np.sum(vel_np * np.log(vel_np + EPS), axis=1)
    press_entropy = -np.sum(press_np * np.log(press_np + EPS), axis=1)
    return {
        "num_shared_experts": int(gate_np.shape[1]),
        "always_active": True,
        "mean_mixer_weight": [float(v) for v in gate_np.mean(axis=0).tolist()],
        "velocity_mean_mixer_weight": [float(v) for v in vel_np.mean(axis=0).tolist()],
        "pressure_mean_mixer_weight": [float(v) for v in press_np.mean(axis=0).tolist()],
        ...
        "velocity_mixer_entropy_mean": float(np.mean(vel_entropy)),
        "pressure_mixer_entropy_mean": float(np.mean(press_entropy)),
    }
...
def model_outputs_from_states_torch(
    ...
    rhs_std, pressure_std, _, _ = model(x)
...
def model_outputs_from_states_np(
    ...
    xt = torch.tensor(scalers["x"].transform(x), dtype=torch.float32, device=device)
    with torch.no_grad():
        rhs_std, pressure_std, _, _ = model(xt)
    rhs_op = (
        rhs_std.detach().cpu().numpy()[0] * scalers["rhs_operator"].scale
        + scalers["rhs_operator"].mean
    )
...
def expert_error_analysis_np(
    ...
    num_experts = int(max(args.num_experts, int(top1.max()) + 1 if len(top1) else 0))
    one_step = {
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, num_train, args.batch_size):
            ...
            if bool(torch.any(smooth_mask)):
                _, _, prev_gates, _ = model(x_all_t[prev_ids[smooth_mask]])
...
def write_summary_md(path: Path, result: Dict[str, object]) -> None:
    ...
    lines = [
        ...
        (
            ...
            f"shared_operator_experts={settings['num_operator_spaces']}, top_k={settings['top_k']}, "
        ...
        "",
        (
            f"Routed and shared experts output `{settings['rhs_target']}` velocity operator "
            f"targets plus a pressure `{settings['pressure_target']}` branch using separate velocity/pressure regime routers. "
            "For `residual`, the learned closure is added to the Galerkin RHS before RK4."
        ),
        "",
        "Losses: one-step coefficient, sampled reconstruction, full-RHS operator, short rollout, "
        "pressure closure, RHS/pressure relative terms, router load-balance, entropy, temporal smoothness, "
        "expert diversity, and weak Re-regime router separation.",
        "",
        "## Metrics",
        "",
    ...
    for item in result["results"]:
        ...
        lines.append(
            f"| {item['test_Re']} | Deep operator-space MoE | {deep['rhs_relative_l2']:.6g} | "
            f"{base['pressure_surrogate_relative_l2']:.6g} | "
    ...
    lines.append("")
    lines.append("| Test Re | shared always active | shared mixer mean weights | shared mixer entropy |")
    lines.append("|---:|---|---|---:|")
    for item in result["results"]:
        shared = item.get("shared_operator_analysis_test", {})
        weights = shared.get("mean_mixer_weight", [])
        weight_text = "[" + ", ".join(f"{float(v):.3f}" for v in weights) + "]" if weights else "-"
        entropy = shared.get("mixer_entropy_mean", float("nan"))
        lines.append(
            f"| {item['test_Re']} | {shared.get('always_active', False)} | "
            f"{weight_text} | {entropy:.6g} |"
        )
    lines.append("")
    lines.append(
...
def main() -> None:
    ...
    out = {
        "scheme": "weighted_l2_deep_moe_rom_v13_re50_300_operator_space_relative_surrogate_rk4",
