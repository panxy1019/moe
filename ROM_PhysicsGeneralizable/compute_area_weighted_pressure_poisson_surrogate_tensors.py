...
其中速度/压力 POD 均来自 Global_POD_AreaWeighted_L2，内积使用
mesh_l2_point_area_weights.npz 中的 point_areas。脚本同时输出伪逆后的等效张量：
...
from compute_area_weighted_l2_semi_intrusive_galerkin_tensors import (
    DEFAULT_DATA_DIR,
    DEFAULT_MESH_VTU,
    POD_DIR_NAME,
    Array,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-dir", type=Path, default=DEFAULT_DATA_DIR, help="area-weighted L2 POD 数据目录")
    parser.add_argument("--mesh-vtu", type=Path, default=DEFAULT_MESH_VTU, help="同网格 VTK/VTU 模板")
...
def write_report(
    ...
    lines.append("- 积分权重使用 area-weighted L2 POD 的 `point_areas`。")
...
def main() -> None:
    ...
    output = args.output
    if output is None:
        suffix = "allRe100_areaWeightedL2" if args.all_re else str(selected_labels[0])
