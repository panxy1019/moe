#!/usr/bin/env python3
"""Deep semi-intrusive operator-space MoE-ROM v11.

V11 changes the V10 residual-correction MoE into an operator-space MoE:
the router selects among local ROM operators, each expert outputs a full
velocity RHS operator, and RK4 advances the velocity coefficients.  The
pressure branch shares the same regime router and predicts a pressure closure
on top of the algebraic pressure surrogate, keeping velocity/pressure coupling
while moving expert semantics away from hidden residual patches.

    [a_t, b_t, Re, phase, physical descriptors, history]
      -> shared encoder h_t
      -> regime router pi_i(h_t, descriptors)
      -> local operator experts f_i
      -> f = sum_i pi_i f_i
      -> RK4 velocity advance + pressure closure
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("/root/moe/V11/test_results_v11/results")
    )
    parser.add_argument("--experiment-name", default="deep_moe_v11")
...
class OperatorSpaceMoEROM(nn.Module):
    ...
    def forward(
        ...
        if self.num_operator_spaces > 0:
            ...
            op = self.routed_scale * routed_op + self.shared_scale * shared_op
        else:
            op = routed_op
...
def model_rhs_torch(
    ...
    return rhs_std * scalers_t["rhs_op_scale"] + scalers_t["rhs_op_mean"]
...
def train_one_split(
    ...
    scalers = {
        ...
        "rhs_operator": Standardizer.fit(arrays["adot"][train_ids]),
    ...
    rhs_train = torch.tensor(
        scalers["rhs_operator"].transform(arrays["adot"][train_ids]), device=device
    )
...
def write_summary_md(path: Path, result: Dict[str, object]) -> None:
    ...
    lines = [
        ...
        (
            ...
            f"operator_spaces={settings['num_operator_spaces']}, top_k={settings['top_k']}, "
        ...
        "",
        f"Experts output full standardized velocity RHS operators plus a pressure `{settings['pressure_target']}` branch using one shared regime router.",
        "",
...
def main() -> None:
    ...
    out = {
        "scheme": "weighted_l2_deep_moe_rom_v11_re50_300_operator_space_relative_surrogate_rk4",
