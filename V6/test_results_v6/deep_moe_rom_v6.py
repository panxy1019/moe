#!/usr/bin/env python3
"""Deep semi-intrusive MoE-ROM v6 experiment with autonomous pressure rollout.

The model extends V5 with a pressure-next head and autonomous-pressure rollout:

    PhysicalContextEncoder -> stacked Shared-Routed MoE blocks -> three heads

The heads predict the next velocity POD state, next pressure POD state, and
the RHS closure correction on top of the semi-intrusive Galerkin RHS.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=Path("/root/moe/V6/results"))
    parser.add_argument("--experiment-name", default="deep_moe_v6")
...
def model_outputs_from_states_torch(
    ...
    b_next = pressure_std * scalers_t["pressure_scale"] + scalers_t["pressure_mean"]
    return rhs_g + corr, b_next, rhs_g
...
def integrate_autonomous_step_torch(
    ...
    k1, b_next, rhs_g = model_outputs_from_states_torch(
...
def rollout_loss_torch(
    ...
    for _ in range(steps):
        ...
        a_next, b_next, rhs_g = integrate_autonomous_step_torch(
            model, a_cur, b_cur, current, dt, a_hist, b_hist, rhs_hist, arrays_t, scalers_t, gal, args
        )
        target_a = arrays_t["a"][nxt]
        target_b = arrays_t["b"][nxt]
        a_loss = F.mse_loss((a_next - target_a) / scalers_t["alpha_scale"], torch.zeros_like(a_next))
        b_loss = F.mse_loss(
            (b_next - target_b) / scalers_t["pressure_scale"], torch.zeros_like(b_next)
        )
...
def model_outputs_from_states_np(
    ...
    return rhs_g + corr, b_next.astype(np.float32), rhs_g
...
def integrate_autonomous_step_np(
    ...
    k1, b_next, rhs_g = model_outputs_from_states_np(
...
def train_one_split(
    ...
    scalers = {
        ...
        "pressure_next": Standardizer.fit(arrays["b_next"][train_ids]),
    }
    ...
    pressure_train = torch.tensor(
        scalers["pressure_next"].transform(arrays["b_next"][train_ids]), device=device
    )
    ...
    rollout_autonomous = rollout_autonomous_pressure_eval_np(
        model, arrays, scalers, tensors, test_label_id, args, device
    )
...
def write_summary_md(path: Path, result: Dict[str, object]) -> None:
    ...
    lines = [
        ...
        "pressure prediction, alpha/RHS consistency, router load-balance, entropy and temporal smoothness.",
        "",
        "## Metrics",
        "",
...
def main() -> None:
    ...
    out = {
        "scheme": "weighted_l2_deep_moe_rom_v6_autonomous_pressure_rk4",
