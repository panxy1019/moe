...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--swanlab-project", default="V14_AdaptivePressureClosure")
...
def swanlab_init_run(
    ...
    try:
        ...
        if len(run_name) > 60:
            run_name = f"v14_apc_{args.closure_mode}"
        kwargs = {
...
def summarize_closure_np(
    ...
    alpha_1d = np.asarray(alpha, dtype=np.float64).reshape(-1)
    beta_1d = np.asarray(beta, dtype=np.float64).reshape(-1)
    base_scale_1d = np.asarray(base_scale, dtype=np.float64).reshape(-1)
    residual_scale_1d = np.asarray(residual_scale, dtype=np.float64).reshape(-1)
        ...
        time_series.append(
            {
                ...
                "alpha": float(alpha_1d[idx]),
                "beta": float(beta_1d[idx]),
                "base_scale": float(base_scale_1d[idx]),
                "residual_scale": float(residual_scale_1d[idx]),
    ...
    return {
        "alpha": stats(alpha_1d),
        "beta": stats(beta_1d),
        "base_scale": stats(base_scale_1d),
        "residual_scale": stats(residual_scale_1d),
        ...
        "alpha_base_error_corr": finite_corr_np(alpha_1d, base_error),
        "alpha_residual_magnitude_corr": finite_corr_np(alpha_1d, residual_mag),
        "beta_base_error_corr": finite_corr_np(beta_1d, base_error),
        "beta_residual_magnitude_corr": finite_corr_np(beta_1d, residual_mag),
...
def evaluate_model(
    ...
    if args.pressure_target == "state":
        ...
        base_component = arrays["pressure_base_next"][sample_ids]
    ...
    return {
        ...
        "pressure_effective_residual_relative_l2": relative_l2_np(
            arrays["pressure_residual"][sample_ids], residual_component
        ),
...
def model_outputs_from_states_torch(
    ...
    return rhs_op, pressure_op, rhs_g, closure_params
        ...
        for start in range(0, num_train, args.batch_size):
            ...
            if args.pressure_target == "state":
                ...
                pressure_next_phy, _, _, _, _ = closure_components_torch(
                    args.closure_mode,
                    arrays_t["pressure_base_next"][batch_ids],
                    pressure_op_phy,
                    closure_params,
                )
...
def main() -> None:
    ...
    if args.pressure_target != "closure":
        raise ValueError("V14_AdaptivePressureClosure requires --pressure-target=closure.")
    if args.pressure_input_mode != "pressure_only":
        raise ValueError(
            "V14_AdaptivePressureClosure keeps the V14 pressure input [a_t,b_t]; "
    ...
    out = {
        "scheme": "v14_adaptive_pressure_closure_hprs_moe_rom_re50_300_closed_loop_rk4",
