#!/usr/bin/env python3
"""Deep semi-intrusive MoE-ROM v7 with pressure-surrogate residual rollout.

The model extends V6 by replacing the direct pressure-next head with a
surrogate-plus-residual pressure update:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--data-root",
        type=Path,
        default=Path("/root/moe/V7/data/Global_POD_Weighted_L2"),
    )
    parser.add_argument(
        "--tensor-path",
        type=Path,
    ...
    parser.add_argument(
        ...
        default=Path(
            "/root/moe/V7/data/"
            "pressure_poisson_surrogate_tensors_allRe30_weightedL2_ru80_rp80.npz"
        ),
    )
    parser.add_argument("--output-dir", type=Path, default=Path("/root/moe/V7/results"))
    parser.add_argument("--experiment-name", default="deep_moe_v7")
...
def main() -> None:
    ...
    out = {
        "scheme": "weighted_l2_deep_moe_rom_v7_pressure_surrogate_residual_rk4",
