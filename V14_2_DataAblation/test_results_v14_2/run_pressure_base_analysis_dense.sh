...
ANALYSIS="${ROOT}/pressure_base_analysis.py"
name="v14_pressure_base_analysis_dense_uniform10"
out="${ROOT}/results/${name}"

mkdir -p "${out}"
echo "===== ${name} train $(date -Is) =====" | tee "${out}/run.log"
"${PY}" "${SCRIPT}" \
  --output-dir "${out}" \
...
echo "===== ${name} pressure-base-analysis $(date -Is) =====" | tee -a "${out}/run.log"
"${PY}" "${ANALYSIS}" \
  --metrics-json "${out}/${name}_metrics.json" \
  --output-dir "${out}/pressure_base_analysis" \
  --experiment-name "${name}" \
  --device cuda 2>&1 | tee -a "${out}/run.log"
