...
mode="${1:?usage: run_v14_3_pressure_input_one.sh pressure_only|velocity_only|hybrid}"
case "${mode}" in
  pressure_only|velocity_only|hybrid) ;;
  *) echo "unknown pressure input mode: ${mode}" >&2; exit 2 ;;
esac
...
SCRIPT="${ROOT}/train_v14_3.py"
name="v14_3_pressure_input_${mode}_dense_uniform10"
