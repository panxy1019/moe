#!/usr/bin/env python3
"""从 weighted L2 POD 数据计算半侵入式 Galerkin 投影张量。

本脚本面向 /home/ray/Desktop/Cylinder_Results_Re500_1000_30Re_POD 中的
Global_POD_Weighted_L2 输出。
速度 POD 文件中的模态布局为 [u(所有点), v(所有点)]，这里会补齐 w=0，形成 (N, 3, r_u)。
压力 POD 是标量模态，形成 (N, r_p)。该数据集的 phi_uv/phi_p 已经是 raw 物理空间模态，
并满足 point_volumes 对应的 lumped mass 正交。
"""
...
DEFAULT_DATA_DIR = Path("/home/ray/Desktop/Cylinder_Results_Re500_1000_30Re_POD")
POD_DIR_NAME = "Global_POD_Weighted_L2"
DEFAULT_MESH_VTU = Path("/home/ray/Desktop/Cylinder_Results_Re500_1000_VTU/Re_1000_VTU/flow_60.vtu")
NU_DEFAULT = 1.0e-3
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="用于拓扑/求导的 VTU 文件；新数据集未保存 VTU，默认使用同网格旧数据的一帧",
    )
...
def load_pod_data(data_dir: Path, ru: int, rp: int) -> PodData:
    """读取 weighted L2 POD 文件，并整理为节点优先的 NumPy 数组。"""

    pod_dir = data_dir / POD_DIR_NAME
    vel_path = pod_dir / "global_velocity_pod_weighted_l2.npz"
    prs_path = pod_dir / "global_pressure_pod_weighted_l2.npz"
    weights_path = pod_dir / "mesh_l2_point_weights.npz"
    if not vel_path.exists() or not prs_path.exists():
        raise FileNotFoundError(f"未找到 POD 文件：{vel_path} 或 {prs_path}")
    if not weights_path.exists():
        raise FileNotFoundError(f"未找到 weighted L2 权重文件：{weights_path}")
    ...
    retained_u = int(vel["retained_rank_uv"])
    retained_p = int(prs["retained_rank_p"])
    ...
    if not np.allclose(points, np.asarray(weights_npz["points"], dtype=np.float64), rtol=0.0, atol=1e-7):
        raise ValueError("POD points 和 mesh_l2_point_weights points 不一致")

    point_volumes = np.asarray(weights_npz["point_volumes"], dtype=np.float64)
    if point_volumes.shape != (n_points,):
        raise ValueError(f"point_volumes 形状异常：{point_volumes.shape}")
    if np.any(point_volumes <= 0.0) or not np.all(np.isfinite(point_volumes)):
        raise ValueError("point_volumes 含非正值或非有限值")
...
def default_vtu_path(data_dir: Path, re_value: float) -> Path:
    ...
    30Re weighted POD 数据集只保存了 uvp npz 和 POD 结果，没有随目录保存 VTU。
    报告中的 mesh_point_match_max_delta=0.0，说明可复用旧 6Re VTU 数据集中的同一套网格拓扑。
    """
...
def validate_point_alignment(mesh: pv.UnstructuredGrid, pod_points: Array) -> None:
    ...
    if mesh_points.shape != pod_points.shape:
        raise ValueError(f"VTU points {mesh_points.shape} 与 POD points {pod_points.shape} 不一致")
    max_abs = float(np.max(np.abs(mesh_points - pod_points)))
    if max_abs > 1e-6:
        raise ValueError(f"VTU 与 POD 点坐标未对齐，max_abs_diff={max_abs:.3e}")
...
def save_npz(
    ...
    metadata = {
        ...
        "data_layout": {
            ...
            "phi_p": "(N, r_p)",
            "mass_weights": "(N,), weighted L2 point_volumes from mesh_l2_point_weights.npz",
        },
        "equations": {
            "raw": "G da/dt = c_raw + A_raw a + H_raw(a,a) + P_raw b",
            "ode": "da/dt = c + A a + H(a,a) + P b, with c/A/H/P = G^{-1} raw tensors",
        },
        "weighted_l2_note": "For this dataset phi_uv/phi_p are raw-space weighted POD modes and are M-orthonormal up to storage precision.",
...
def write_report(
    ...
    lines: list[str] = []
    lines.append("# Weighted L2 Semi-intrusive Galerkin Projection Tensors\n")
    lines.append("## 数据来源\n")
    ...
    lines.append("```\n")
    lines.append("本数据集已经提供 `Global_POD_Weighted_L2/mesh_l2_point_weights.npz`，脚本直接读取其中的 `point_volumes` 作为 `mass_weights`。这些权重来自同一套 blockMesh 六面体单元体积按角点均分后的 nodal control volumes，与 weighted L2 POD 报告中的内积一致。\n")
    lines.append("与上一版 unweighted 数据集不同，本次 `phi_uv/phi_p` 是 raw 物理空间 weighted POD 模态，并满足质量矩阵正交：`Phi^T M Phi ≈ I`。脚本仍保存 `G_u` 和 Gram 修正后的 `c/A/H/P`，用于消除 float32 存储和导出误差带来的小偏差。\n")
    ...
    lines.append("```\n")
    lines.append("对 weighted L2 raw-space 模态，理论上 `G≈I`。为保持数值严格性，脚本先装配：\n")
    lines.append("```text")
    ...
    lines.append("```\n")
    lines.append("其中 `a` 是速度模态系数，`b` 是压力模态系数；`c/A/H/P` 对应 `.npz` 中的 `Re_xxx_c`、`Re_xxx_A`、`Re_xxx_H`、`Re_xxx_P`。\n")
...
def main() -> None:
    ...
    output = args.output
    if output is None:
        suffix = "allRe30_weightedL2" if args.all_re else str(re_labels_selected[0])
    ...
    print(f"[{timer.stamp()}] 读取 weighted L2 point_volumes 作为质量权重", flush=True)
