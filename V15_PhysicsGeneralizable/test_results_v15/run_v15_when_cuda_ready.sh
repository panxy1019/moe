...
AGG_DIR="${ROOT}/results/V15_summary"
...
while true; do
  if "${PY}" - <<'PY' >/tmp/v15_cuda_check.txt 2>&1
import torch
...
PY
  then
    echo "CUDA ready $(date -Is): $(cat /tmp/v15_cuda_check.txt)" | tee -a "${log}"
    exec bash "${ROOT}/run_v15_all.sh"
  fi
  echo "CUDA unavailable $(date -Is); sleeping ${CHECK_INTERVAL_SECONDS}s" | tee -a "${log}"
  cat /tmp/v15_cuda_check.txt | tail -n 4 >> "${log}" || true
