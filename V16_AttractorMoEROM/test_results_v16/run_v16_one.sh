...
case_name="${1:?usage: run_v16_one.sh V16_AttractorMoEROM_FullRegimeLoss32|V16_AttractorMoEROM_AttractorConditionedFramework32}"

ru=32
rp=32
extra_args=()
lambda_attr_ce="0.0"
lambda_proto_energy="0.0"
lambda_proto_radius="0.0"

case "${case_name}" in
  V16_AttractorMoEROM_FullRegimeLoss32)
    ;;
  V16_AttractorMoEROM_AttractorConditionedFramework32)
    extra_args+=(--attractor-conditioned --attractor-adapter-dim 64)
    lambda_attr_ce="0.02"
    lambda_proto_energy="0.01"
    lambda_proto_radius="0.01"
    ;;
  *)
    echo "unknown V16 case: ${case_name}" >&2
    exit 2
    ;;
esac
...
SCRIPT="${ROOT}/train_v16_attractor_moe_rom.py"
name="${case_name}_ru${ru}_rp${rp}"
out="${ROOT}/results/${case_name}/${name}"
TRACKING_MODE="${SWANLAB_TRACKING_MODE:-online}"
TRACKING_PROJECT="${SWANLAB_TRACKING_PROJECT:-V16_AttractorMoEROM}"
TRACKING_GROUP="${SWANLAB_TRACKING_GROUP:-v16-attractor-moe-rom}"
...
if not torch.cuda.is_available():
    print("CUDA is not available; refusing to run V16 training on CPU.", file=sys.stderr)
    sys.exit(3)
...
"${PY}" "${SCRIPT}" \
  ...
  --lambda-attractor-ce "${lambda_attr_ce}" \
  --lambda-proto-energy "${lambda_proto_energy}" \
  --lambda-proto-radius "${lambda_proto_radius}" \
  ...
  --weight-decay 1.5e-4 \
  --seed "${V16_SEED:-1600}" \
  --epochs "${V16_EPOCHS:-240}" \
  --min-epochs "${V16_MIN_EPOCHS:-130}" \
  --patience 70 \
  --eval-every "${V16_EVAL_EVERY:-5}" \
